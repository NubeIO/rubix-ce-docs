...
4. **Step-4** Once all the settings are added click on **Create Network** button. This will now create the Modbus and network. Now open the Modbus network to add a device.
...
You must have a Rubix Compute and a Modbus RTU device and know the settings required to add the device **[User Manual](../../../../hardware/controllers/supervisors/rubix-compute/user-manual.md)**
:::
...
| Description | Description of the device, eg: `temp 1` |
|Host IP| Fill in for Modbus IP only, IP of the device you wanted to connect with|
| Port              | Default port is `502` you can also set it to another port if required.    |
